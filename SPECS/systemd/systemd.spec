...
%endif
%if %{undefined rhel} && %{undefined azure}
...
%build
%global ntpvendor %(. /etc/os-release; echo ${ID})
...
CONFIGURE_OPTS=(
        ...
        -Ddev-kvm-mode=0666
        -Dkmod=enabled
        -Dxkbcommon=%[%{defined azure}?"disabled":"enabled"]
        -Dblkid=enabled
        -Dfdisk=enabled
        -Dseccomp=enabled
        -Dima=true
        ...
        -Dapparmor=disabled
        -Dpolkit=%[%{defined azure}?"disabled":"enabled"]
...
%endif
        ...
        -Delfutils=enabled
        -Dpwquality=enabled
        -Dqrencode=%[%{defined rhel} || %{defined azure}?"disabled":"enabled"]
...
%if ! (0%{?fedora} >= 39 || 0%{?rhel} >= 10 || 0%{?azure} >= 3)
...
# Unfortunately Mariner currently refuses to allow a custom clean section
#%clean
...
# changelog here.
%changelog
* Thu Jan  4 16:13:06 EST 2024 Dan Streetman <ddstreet@ieee.org> - 255-2
...
- Changelog following this is from upstream Fedora package; previous Photon/Azure package changelog is at the bottom.
